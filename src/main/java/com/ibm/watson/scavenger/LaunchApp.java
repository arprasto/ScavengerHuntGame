...
public class LaunchApp {
	...
	{
		if(args.length <= 1){
			System.out.println("Usage\n com.ibm.watson.scavenger.LaunchApp [properties file path] ['train' | 'scavenger']");
			System.exit(0);
		}
	...
	if(args[1].toLowerCase().trim().equals("train")){
		ImageTrainingApp.getInstance().startTrainingApp();
	}else if(args[1].toLowerCase().trim().equals("scavenger")){
		PredictionApp.getInstance().startGame();
	}else{
		JOptionPane.showMessageDialog(null,"app name can be one of 'train' or 'scavenger'","ApplicationError",JOptionPane.ERROR_MESSAGE);
		System.exit(0);
	}
	}
